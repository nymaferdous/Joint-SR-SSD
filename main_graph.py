...
def train():
    ...
    # sess1 = tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False))
    ...
    # #
    # reader = pywrap_tensorflow.NewCheckpointReader('/home/nyma/PycharmProjects/JointSSD/checkpoint_128/SRGAN_weight_X4')
    # var_to_shape_map = reader.get_variable_to_shape_map()
    # print("Checkpoint Variableas", var_to_shape_map)
    #
    ...
    # loader = tf.train.Saver(g_vars)
    # saver= tf.train.Saver(g_vars, max_to_keep=10)
    # loader.restore(sess, initial_weight_srgan)
    ...
    for epoch in range(0, n_epoch + 1):
        ...
        for idx in range(0, len(train_hr_img_list), batch_size):
            ...
            g2 = tf.Graph()
            with g2.as_default():
                t_image = tf.placeholder('float32', [batch_size, in_size, in_size, 3],
                                         name='t_image_input_to_SRGAN_generator')
                t_target_image = tf.placeholder('float32', [batch_size, 256, 256, 3], name='t_target_image')
                t_target_image512 = tf.placeholder('float32', [batch_size, 512, 512, 3], name='t_target_image')

                sess1 = tf.Session()
                net = SSDGRAPH(sess1, preset)
                net.build_from_vgg(args.vgg_dir, source.num_classes)
                net.build_optimizer()
                initialize_uninitialized_variables(sess1)
                result, loss_batch, _ = sess1.run([net.result, net.losses,
                                                   net.optimizer],
                                                  {net.image_input: p[0], t_image: b_imgs_96,
                                                   t_target_image512: b_imgs_384,
            ...
            #       %(epoch, n_epoch, n_iter, time.time() - step_time, loss_batch['total']))
            #
            ...
            #
            #     if len(training_imgs_samples) < 3:net
            ...
            #     [train_op_srgan, g_loss + 1e-3 * net.losses['total'],  mse_loss, vgg_loss, g_gan_loss, mse_loss512, vgg_loss512, g_gan_loss512],
            #     {net.image_input: p[0], t_image: b_imgs_96, t_target_image: b_imgs_256, t_target_image512: b_imgs_384,net.labels: y })
            # _, errG, errM, errV, errA, errM_512, errV_512, errA_512 = sess.run(
