...
def conv_map(x, size, shape, stride, name, padding='SAME'):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        #                         shape=[shape, shape, 3, size],
        #                         initializer=tf.contrib.layers.xavier_initializer())
        # if name=='conv1_2':
        #     w = tf.get_variable("filter",
        #                         shape=[shape, shape,64, size],
        #                         initializer=tf.contrib.layers.xavier_initializer())
        # if name=='conv2_1':
        #     w = tf.get_variable("filter",
        #                         shape=[shape, shape,64, size],
        ...
        #                         initializer=tf.contrib.layers.xavier_initializer())
        # if name=='mod_conv6':
        #     w = tf.get_variable("filter", dtype=tf.float32, trainable=True,
        #                         shape=[3, 3, 512, 1024],
        #                         initializer=tf.contrib.layers.xavier_initializer())
        # if name =='mod_conv7':
        #     w = tf.get_variable("filter", dtype=tf.float32, trainable=True,
...
def conv_map_fc6(x, size,stride,name, padding='SAME'):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        w = tf.get_variable("filter",
                            shape=[3, 3, 512, 512],
                            initializer=tf.contrib.layers.xavier_initializer())
...
def conv_map_fc7(x, size, stride, name, padding='SAME'):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        w = tf.get_variable("filter",
                            shape=[1, 1, 512, 1024],
                            initializer=tf.contrib.layers.xavier_initializer())
...
def classifier(x, size, mapsize, name):
    with tf.variable_scope(name):
        ...
        w = tf.get_variable("filter",
                            ...
                            initializer=tf.contrib.layers.xavier_initializer())
        print("Classifier", x.get_shape()[3])
        b = tf.Variable(tf.zeros(size), name='biases')
...
class SSDVGG:
    #---------------------------------------------------------------------------
    def __init__(self, image_input, preset, trainable, num_classes):
    ...
    def build_from_vgg(self, vgg_dir, num_classes, a_trous=True,
        ...
        self.__build_norms()
        self.__select_feature_maps()
        self.result = self.__build_classifiers()
        self.__built = True
        # return self.result

    #---------------------------------------------------------------------------
    def build_from_metagraph(self, sess, metagraph_file, checkpoint_file):
        """
        ...
        with tf.Session() as sess:
            ...
            self.image_input = sess.graph.get_tensor_by_name('t_target_image:0')
    ...
    def __build_ssd_layers(self):
        ...
        x, l2 = conv_map(self.ssd_conv5_3, 1024, 3, 1, 'mod_conv6')
        self.mod_conv6 = self.__with_loss(x, l2)
        x, l2 = conv_map(self.mod_conv6, 1024, 1, 1, 'mod_conv7')
        ...
        # x, l2 = conv_map(self.ssd_conv1_1, 64, 3, 1, 'conv1_2')
        # self.ssd_conv1_2 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv1_2, 128, 3, 1, 'conv2_1')
        # self.ssd_conv2_1 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv2_1, 128, 3, 1, 'conv2_2')
        # self.ssd_conv2_2 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv2_2, 256, 3, 2, 'conv3_1')
        ...
        # x, l2 = conv_map(self.ssd_conv3_2, 256, 3, 1, 'conv3_3')
        # self.ssd_conv3_3 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv3_3, 256, 3, 2, 'conv4_1')
        # self.ssd_conv4_1 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv4_1, 256, 3, 1, 'conv4_2')
        # self.ssd_conv4_2 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv4_2, 512, 3, 1, 'conv4_3')
        # self.ssd_conv4_3 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv4_3, 512, 3, 2, 'conv5_1')
        ...
        # x, l2 = conv_map(self.ssd_conv5_2, 512, 3, 1, 'conv5_3')
        # self.ssd_conv5_3 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv5_3, 1024, 1, 1,'mod_conv6')
        # self.mod_conv6 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.mod_conv6, 1024, 1, 1, 'mod_conv7')
        # self.mod_conv7 = self.__with_loss(x, l2)
        # self.mod_pool5 = tf.nn.max_pool(self.ssd_conv5_3, ksize=[1, 3, 3, 1],
        #                                 strides=[1, 1, 1, 1], padding='SAME',
        #                                 name='mod_pool5')
        # print("After modpool5", self.mod_pool5.shape)
        # #
        # with tf.variable_scope('mod_convo6'):
        #     x = tf.nn.conv2d(self.mod_pool5, self.vgg_fc6_w,
        #                      strides=[1, 1, 1, 1], padding='SAME')
        #
        #     # x = tf.nn.conv2d(self.vgg_conv5_3, self.vgg_fc6_w,
        ...
        #     self.l2_loss += tf.nn.l2_loss(self.vgg_fc6_w)
        # #
        # with tf.variable_scope('mod_convo7'):
        #     x = tf.nn.conv2d(self.mod_conv6, self.vgg_fc7_w,
        #                      strides=[1, 1, 1, 1], padding='SAME')
        #     x = tf.nn.bias_add(x, self.vgg_fc7_b)
        #     x = tf.nn.relu(x)
        #     self.mod_conv7 = x
        #     self.l2_loss += tf.nn.l2_loss(self.vgg_fc7_w)
        # x, l2 = conv_map(self.mod_conv7, 256, 1, 1, 'convo8_1')
        # self.ssd_conv8_1 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv8_1, 512, 3, 2, 'convo8_2')
        # self.ssd_conv8_2 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv8_2, 128, 1, 1, 'convo9_1')
        # self.ssd_conv9_1 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.ssd_conv9_1, 256, 3, 2, 'convo9_2')
        # self.ssd_conv9_2 = self.__with_loss(x, l2)
        ...
        # x, l2 = conv_map(self.ssd_conv10_1, 256, 3, 2, 'conv10_2', 'SAME')
    ...
    def __build_norms(self):
        ...
        self.norm_conv4_3 = x
        # x = l2_normalization(self.ssd_conv4_2, 20, 256, 'l2_norm_conv4_2')
        # self.norm_conv4_2 = x
    ...
    #---------------------------------------------------------------------------
    # def build_optimizer(self, learning_rate=0.001, weight_decay=0.005,
    #                     momentum=0.2, global_step=None):
    #
    ...
    #     # Build the optimizer
    #     #-----------------------------------------------------------------------
    #     with tf.variable_scope('optimizer_ssd',reuse=tf.AUTO_REUSE):
    #
    #         detector_vars1 = []
    #         detector_vars2 = []
    ...
    #             first_stage_optimizer = tf.train.AdamOptimizer(learning_rate).minimize(self.loss,
    ...
    #             second_stage_trainable_var_list = detector_vars2
    #             second_stage_optimizer = tf.train.AdamOptimizer(learning_rate).minimize(self.loss,
    ...
    #         # optimizer = optimizer.minimize(self.loss, global_step=global_step,var_list=detector_vars,
    #         #                                name='optimizer_ssd')
    #
    #     #-----------------------------------------------------------------------
    #     # Store the tensors
    ...
    #
    #     self.optimizer = train_op_with_frozen_variables
    #     self.losses = {
    #         'total': self.loss,
    #         'localization': self.localization_loss,
    #         'confidence': self.confidence_loss,
    ...
    def compute_loss(self, labels, num_classes, global_step =None, weight_decay=10e-15, momentum=0.5, learning_rate=10e-6):
        ...
        with tf.variable_scope('ground_truth'):
            ...
            # Classification ground truth tensor
            # Shape: (batch_size, num_anchors, num_classes)
            gt_cl = labels[:, :, :num_classes+1]

            # Localization ground truth tensor
            # Shape: (batch_size, num_anchors, 4)
            gt_loc = labels[:, :, num_classes+1:]
        ...
        with tf.variable_scope('total_loss'):
            ...
            for var in tf.trainable_variables():
                ...
                if var_name_mess[0] in ['conv1_1', 'conv1_2', 'conv2_1', 'conv2_2', 'conv3_1', 'conv3_2', 'conv3_3',
                                        ...
                                        'conv8_1','conv8_2','conv9_1','conv9_2','classifiers'
                                        ]:
                    detector_vars1.append(var)
            ...
            #     optimizer = tf.train.MomentumOptimizer(learning_rate, momentum)
            #     optimizer = optimizer.minimize(self.loss, global_step, name='optimizer')
        ...
        #         first_stage_trainable_var_list = detector_vars1
        #         # print(first_stage_trainable_var_list)
        #         first_stage_optimizer = tf.train.AdamOptimizer(lr_v).minimize(self.loss,
        #                                                                                var_list=first_stage_trainable_var_list)
        #
        #         with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        #             with tf.control_dependencies([first_stage_optimizer]):
    ...
    def __build_names(self):
        ...
        self.new_scopes = [
            'conv8_1', 'conv8_2', 'conv9_1', 'conv9_2'
        ]
    ...
    #---------------------------------------------------------------------------
    def build_summaries(self, sess, restore):
        if restore:
            return sess.graph.get_tensor_by_name('net_summaries/net_summaries:0')
