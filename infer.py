...
def main():
    ...
    parser.add_argument('--name', default='Testcheckpoint',
                        help='project name')
    ...
    parser.add_argument('--batch-size', type=int, default=4,
                        help='batch size')
    parser.add_argument('--sample', default='trainval',
    ...
    if not os.path.exists(metagraph_file):
        ...
        return 1
    print("Meta Graph File", metagraph_file)
    #---------------------------------------------------------------------------
    ...
    with tf.name_scope('image_input'):
        image_input = tf.placeholder(dtype=tf.float32, name='image_input')
    ...
    # saver.restore(sess, cfg.TEST.WEIGHT_FILE)
    ...
    net = SSDVGG(image_input, preset, trainable, num_classes)
    net.build_from_vgg(args.vgg_dir, num_classes, a_trous=True, progress_hook='tqdm')
    with tf.Session() as sess:
        initialize_uninitialized_variables(sess)
        # sess.run(init_op)
        print('[i] Creating the model...')
        saver = tf.train.Saver()
        saver.restore(sess, '/home/nyma/PycharmProjects/JointSSD/Testcheckpoint/e2.ckpt')

        # net.build_from_metagraph(sess, metagraph_file, checkpoint_file)
