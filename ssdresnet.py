...
class SSDRESNET:
    #---------------------------------------------------------------------------
    def __init__(self,image_input,trainable, num_classes, depth):
        layers = []
        self.__built = True
    ...
    def build_from_vgg(self, vgg_dir, num_classes, a_trous=True,
        ...
        self.__build_ssd_layers(vgg_dir)
        self.__build_norms()
        self.__select_feature_maps()
    ...
    def __build_ssd_layers(self, n):
        ...
        with tf.variable_scope('conv1'):
            conv1, l2 = conv_layer(self.image_input, [3, 3, 3, 16], 1)
            self.ssd_conv1_1 = self.__with_loss(conv1, l2)
            print(conv1.shape)
            layers.append(conv1)

        for i in range(num_conv):
            with tf.variable_scope('conv2_%d' % (i + 1)):
                conv2_x, l2 = residual_block(layers[-1], 16, False)
                conv2, l2 = residual_block(conv2_x, 16, False)
                layers.append(conv2_x)
                layers.append(conv2)
                self.ssd_conv2_1 = self.__with_loss(conv2, l2)

            assert conv2.get_shape().as_list()[1:] == [512, 512, 16]
        ...
        for i in range(num_conv):
            ...
            assert conv3.get_shape().as_list()[1:] == [256, 256, 32]
        ...
        for i in range(num_conv):
            ...
            assert conv4.get_shape().as_list()[1:] == [128, 128, 64]
        ...
        #         layers.append(conv5)
        #
        #     assert conv4.get_shape().as_list()[1:] == [64, 64, 64]
        #
        # for i in range(num_conv):
        #     down_sample = True if i == 0 else False
        #     with tf.variable_scope('conv6_%d' % (i + 1)):
        #         conv6_x = residual_block(layers[-1], 64, down_sample)
        #         conv6 = residual_block(conv5_x, 64, False)
        #         layers.append(conv5_x)
        #         layers.append(conv5)
        #
        #     assert conv4.get_shape().as_list()[1:] == [64, 64, 64]
        ...
        # x, l2 = conv_map(self.ssd_conv5_3, 512, 3, 1, 'mod_conv6')
        # self.mod_conv6 = self.__with_loss(x, l2)
        # x, l2 = conv_map(self.mod_conv6, 1024, 3, 1, 'mod_conv7')
        # self.mod_conv7 = self.__with_loss(x, l2)

        # x, l2 = conv_map_fc6(self.ssd_conv5_3, 512, 1, 'mod_conv6')
        # self.mod_conv6 = self.__with_loss(x, l2)
        # #
        # x, l2 = conv_map_fc7(self.mod_conv6, 1024, 1, 'mod_conv7')
        # self.mod_conv7 = self.__with_loss(x, l2)
        ...
        x, l2 = conv_map(layers[18], 512, 1, 1, 'conv8_1')
    ...
    def __build_norms(self):
        x = l2_normalization(layers[18], 20, 256, 'l2_norm_conv3_3')
        self.norm_conv3_3 = x
        x = l2_normalization(layers[16], 20, 512, 'l2_norm_conv4_3')
        self.norm_conv4_3 = x
    ...
    def compute_loss(self, labels, num_classes, weight_decay):
